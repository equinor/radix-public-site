...
- Calling the API directly ([Radix Platform API](https://api.radix.equinor.com/swaggerui/index.md) or [Radix Playground API](https://api.playground.radix.equinor.com/swaggerui/)), by passing the bearer token (i.e. curl -X GET --header "Authorization: Bearer \$token")
