...
Job can be run as a single job or as a batch of jobs.
...
Get a list of all single jobs with their states by sending a `GET` request to `http://compute:8000/api/v1/jobs`. The response is an array of job state objects, similar to the response received when creating a new job. Jobs, started within a batch are not included in this list 
...
- `ended` is the date and time the batch successfully ended (it does not exist in the response, when empty). Also represented in RFC3339 form and is in UTC. This value is only set for `Successful` batches. Batch is ended when all single jobs it started are completed of failed. 
