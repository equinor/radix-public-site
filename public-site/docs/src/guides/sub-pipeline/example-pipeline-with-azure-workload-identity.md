...
::: tip
The `issuer` URL will change in certain cases when a Radix Cluster is replaced during upgrading or disaster recovery, but we will notify ahead of time if when this happens in the #omnia_radix slack channel.
:::
