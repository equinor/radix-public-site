...
[Deployments](/start/radix-concepts/index.md#deployment) in Radix are portable, which makes it possible to use them in several scenarios.

Usually deployments are created by a `build-deploy` [job](/start/radix-concepts/index.md#job) and deployed into an [environment](/start/radix-concepts/index.md#environment). Older deployments remain accessible under the "Deployments" section of the Web Console.
