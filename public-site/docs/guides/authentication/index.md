...
- Build the application in Radix and open the Radix Web Console to set REDIS_PASSWORD for the `redis` component, and required secrets for the OAuth service used by the `web` component.
  ...
  - Open the `web` component configure secrets required by the OAuth service.  
    ...
    `Client Secret` - see details below. It is only applicable when `authentication.oauth2.useAzureIdentity: false` or not set, otherwise the secret is not shown.
...
- Configure Workload Identity authentication for the OAuth2 proxy by setting `useAzureIdentity: true` in the [oauth2](/radix-config#oauth2) section in [radixconfig.yaml](/radix-config/index.md)
