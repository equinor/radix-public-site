...
When OAuth2 is enabled for a component in [`radixconfig.yaml`](/docs/radix-config/index.md#oauth2), Radix creates an OAuth2 service and configures this service to verify if incoming requests are authorized or not, handle the authorization code flow, and manage session state for an authorized user.
