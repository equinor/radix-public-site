...
When you add `monitoring: true` to [`radixconfig.yaml`](/docs/references/reference-radix-config/#public), Radix will scrape the `/metrics` endpoint on your application expecting metrics in the `Prometheus` format (this is a very simple text-based format).
